...
# --- 4. テキストデータの読み込み ---
def get_saved_text(year, month, week_label):
    """保存シートから 西暦-月-週 が一致する行を探す"""
    try:
        ...
        # 検索キー (例: "2026-4月-W1")
        search_key = f"{year}-{month}-{week_label.split()[0]}"
        
        for row in rows[1:]:
            if len(row) > 0 and search_key in str(row[0]):
...
def save_to_sheet(year, month, week_label, zasu, tanka, cvr, kyaku, summary):
    all_data = ws.get_all_values()
    target_row = -1
    save_key = f"{year}-{month}-{week_label.split()[0]}"
    
    for i, row in enumerate(all_data):
        if row and save_key == str(row[0]):
            target_row = i + 1
            break
            
    new_row = [save_key, zasu, tanka, cvr, kyaku, summary]
    if target_row != -1:
...
sel_week = st.sidebar.selectbox("週", list(week_map.keys()))
row_idx = week_map[sel_week]

# データロード
current_gid = MONTH_CONFIG[sel_year][sel_month]["gid"]
df_raw = load_raw_data(current_gid)
current_txt = get_saved_text(sel_year, sel_month, sel_week)

# 入力フォーム
with st.sidebar.form("input_form"):
    st.write(f"✍️ {sel_month} {sel_week} のコメント編集")
    ...
    if st.form_submit_button("全ユーザーに共有保存"):
        save_to_sheet(sel_year, sel_month, sel_week, r_zasu, r_tanka, r_cvr, r_kyaku, sum_text)
        st.cache_data.clear()
        st.rerun()
...
if not df_raw.empty:
    ...
    <style>
        ...
        .comment-cell {{ text-align: left !important; background-color: #fdfcf7 !important; white-space: pre-wrap; vertical-align: middle; }}
    ...
    st.warning("指定された月のデータを読み込めませんでした。")
